...
// Ejercicio 2
int initArray(
        arrayLength_t *miArray) {      //Funcion que recibe por parametro un puntero a una estructura del tipo arrayLength_t
...
//Ejercicio 4
int getArrSize(arrayLength_t *miArray) {
    int i;
    // Procedemos a crear un bucle para recorrer el Array
    for (i = 0; i < SIZE; i++){
        printf("%d",miArray[i]); // Para imprimir el contenido del array, realizamos un printf para mostrar en pantalla.
    }
    if (miArray == NULL) {
...
//Ejercicio 5
int getArrAdd(arrayLength_t *miArray){//Declaramos otra estructura, con nombre getArrAdd
    int suma=0;//Damos valores iniciales a la suma del array.
//Para recorrer el array y ver los elementos almacenados en el mismo, utilizaremos un buble for o un do-while
    for (int i=0;i<SIZE;i++){
        suma = suma + miArray[i].arrInt;
    }
    printf("El resultado de la suma de los contenidos del array es: %d",suma);
